...
curl -d '{"id": 2, "title": "json-server-2", "author": "mario"}' -H "Content-type: application/json" -X PUT http://localhost:3000/profile/
...
curl -d '{"author": "Adilma"}' -H "Content-type: application/json" -X PATCH http://localhost:3000/posts/1

// DELETE
? Descrição
* Nesta aula falaremos sobre o método DELETE, que serve para remover um recurso, e pode ser respondido com o 
* código 202, que não foi processado mas já foi aceito, 204, que significa que o recurso não existe e 200, 
* que significa que o conteúdo foi deletado. Ele não é um método seguro, mas é idempotente, tem a 
* possibilidade de receber Body tanto na requisição quanto na resposta, não é usado em formulários e não 
* é cacheable.
...
? Descrição
* Para facilitar nossos estudos vamos dividir os Headers por contextos. Para ver as categorias em prática, 
* abra o DevTools na aba Network e abra o site do Google, onde você poderá clicar no primeiro campo e ver 
* que aparecem 3 listas: General, Response Headers e Request Headers. A maioria dos frameworks já vem com 
* alguns headers pré-estabelecidos, mas durante esse módulo vamos ensinar também como se aprofundar neste 
* conteúdo e procurar mais conhecimento.

//General
? Descrição
* Na parte de General, podemos perceber que são Headers que servem tanto para o pedido quanto para a resposta, 
* ou seja, é um agrupamento geral. Dentro deste grupo tempos o Request URL, que é o local do site, o Request 
* Method, que é o método do pedido, o Status Code, o Remote Address, que é um IP que pode ser entendido como 
* quase um endereço físico, e por fim temos o Referrer Policy, que é uma política para o referenciado, ou 
* seja, são políticas para o redirecionamento.


// Request
? Descrição
* Se abrirmos a aba dos cabeçalhos de request, podemos ver vários headers, como o :authority:, que é a autoridade 
* do pedido, o :method:, que é o método do pedido, o :path:, que é o caminho do pedido, o :schema:, que é o 
* esquema que foi usado, o accept, que é o que é aceito, accept-encoding, que são os tipos de compressão usados, 
* cookie, que são rastros deixados para a próxima conexão, entre outros.

// Response
? Descrição
* Se abrirmos a aba dos cabeçalhos de request, podemos ver vários headers, como o :authority:, que é a 
* autoridade do pedido, o :method:, que é o método do pedido, o :path:, que é o caminho do pedido, 
* o :schema:, que é o esquema que foi usado, o accept, que é o que é aceito, accept-encoding, que são os 
* tipos de compressão usados, cookie, que são rastros deixados para a próxima conexão, entre outros.

// Obtendo Ajuda
? Descrição
* Para obter mais conhecimento nessa parte, nós recomendamos o app DevDocs, que serve para buscar informações 
* sobre diversos frameworks, headers, etc. Tudo está em inglês, mas pode-se ir usando ferramentas de tradução 
...
* 200 em si, significa que tudo está ok (GET e POST), 201, significa que o recurso foi criado (PUT), 
...
* 308, significa redirecionamento permanente, 302, significa uma mudança temporária assim como o 307. 
...
* 404, que o pedido não foi encontrado, 405, que significa que o método usado não é permitido e 
* 429, que significa que foram efetuados muitos pedidos. Os do tipo 500: 500, que significa que ocorreu 
* um erro desconhecido no servidor e o 503, que significa que o servidor está indisponível no momento.
...
* devdocs.io, e nele pode ser pesquisado algo como "http status" e pode-se encontrar diversos status code 
* junto aos seus significados.
